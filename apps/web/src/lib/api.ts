...
export class ApiClient {
  ...
  handleGoogleCallback(code: string): Promise<DataResponse<GoogleCallbackResponse>> {
    return this.get(`/api/integrations/google/callback?code=${encodeURIComponent(code)}`);
  }
  ...
  getGoogleCalendarEvents(maxResults?: number): Promise<DataResponse<GoogleCalendarEvent[]>> {
    return this.get(`/api/integrations/google/events${this.qs({ maxResults })}`);
  }
  ...
  }): Promise<DataResponse<{ business: Business; script: Script }>> {
